...
const DeleteLocataireById = async (req, res) => {
  ...
  try {
    ...
    const result1 = await db.sequelize.query(`UPDATE appel_offres SET id_locataire =null WHERE id_locataire = '${LocataireID}'`, { type: QueryTypes.UPDATE })
    // If the appel_offre was successfully Updated, return a success response
    try {
      // Find the Locataire by ID and delete it
  
      const result1 = await db.sequelize.query(`DELETE FROM locataire WHERE id_locataire = '${LocataireID}'`, { type: QueryTypes.UPDATE })
      // If the appel_offre was successfully Updated, return a success response
      if (Appel) {
      ...
      // If the Locataire ID was not found, return a 404 response
